...
**AI-Powered Entity & Relationship Intelligence**
...
Every large division faces the same invisible problem: **your most critical data about people, projects, and performance lives in disconnected documents** — spreadsheets, review packets, goal trackers, org charts, email threads. Leaders make talent decisions, allocate resources, and plan succession based on incomplete pictures assembled manually from dozens of sources.

Consider a real scenario:

- A division head runs a 30-person organization with $15M in budget across 5 major initiatives
- The **annual talent review** spans 30 employees, 120+ goals, 90+ peer reviews, retention risks, succession plans, and cross-team dependencies
- **One question** — *"If our top engineer leaves, what breaks?"* — requires mentally tracing connections across people, projects, skills, and reporting lines that no single document captures
...
- Run **What-If simulations** — *"What happens if we lose our top engineer?"* — and watch AI agents model the ripple effects across the organization
...
Any division at scale — whether it's Operations, Finance, Legal, Compliance, or Technology — deals with the same complexity. A 30-person organization working on 5 major cross-functional initiatives has a talent landscape like this:

| Dimension | Scale |
|---|---|
| Employees | 30 (senior leadership → junior analysts, across multiple locations) |
| Teams | 5 functional groups with cross-team dependencies |
| Annual Initiatives | 5 ($10M+ combined budget) |
| Individual Goals | 120+ (4-5 per employee) |
| 360 Reviews | 90+ feedback entries (peers, managers, skip-level, stakeholders) |
| Retention Risks | Multiple flagged (some actively recruited externally) |
| Succession Plans | Critical leadership positions mapped |
...
When you feed a division's talent review into Orion, it automatically discovers:

- **People <> Project connections**: A key engineer is the technical lead on one $4M initiative AND leads two other programs — a 3x single point of failure
- **Cross-team dependencies**: A $1.8M initiative depends on one team's platform work AND another team's governance review — a bottleneck invisible in any org chart
- **Cascading risks**: If a high-retention-risk employee leaves, it impacts multiple projects, breaks mentoring relationships, and derails deliverables across teams
- **Goal alignment gaps**: A security reviewer is a dependency for 2 of 5 initiatives — but their own goal is to reduce review turnaround time, creating tension between thoroughness and speed
...
Ask: *"What if our top ML engineer accepts a competitor offer?"*

Orion's AI agents roleplay as each affected entity — modeling how the manager reassigns work, how project timelines slip, who can absorb ownership, and what the budget impact looks like. You can then **interview individual agents** to drill deeper: *"What would it take to retain this person?"*
...
```
  Documents (PDF, XLSX, DOCX, PPTX, Images, Email, CSV, TXT)
     |
     v  LLM Vision OCR (for PDFs, images, presentations)
     |
     v
+-----------------+         +-----------------+         +-----------------+
...
|                 |         |                 |         |   Merges typos, |
| Reads data,     |         | Extracts entities|        |   abbreviations |
| infers entity & |         | & relationships |         |   into canonical|
| relationship    |         | per chunk (live) |         |   forms         |
| types           |         |                 |         |                 |
+-----------------+         +-----------------+         +-----------------+
                                                                |
                                                                v
...
Orion uses **LLM Vision OCR** — an enterprise-grade document extraction engine — for PDFs, images, and presentations. Every page is rendered as a high-resolution image and processed by the multimodal LLM, capturing:
...
- **80+ nodes**: Performance mode — shadows off, straight edges, reduced physics for smooth interaction
- Physics engine freezes after stabilization to eliminate ongoing CPU usage
- Edges hidden during drag and zoom for responsive interaction
...
- **LLM Connection**: At least one LLM (e.g., GPT-4o, Claude). Multimodal model recommended for Vision OCR.
...
4. Explore: Click on any employee to see their project dependencies and review feedback
5. Switch to **What-If** tab — try scenarios like key departures, reorgs, or budget cuts
6. Interview the agents: Click on any entity card to ask follow-up questions in character
